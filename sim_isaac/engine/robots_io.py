...
# lease of the lease running out. The same tick records which robots hold
# the pairs their models ask for, which is what renews a lease.
...
class RobotsIO:
    ...
    def stand(self, name: str, known: IsaacModel, placement: Placement) -> None:
        """Stands a robot of the model `known` and resolves the scene around
        it, on the thread that steps the scene. A robot the engine cannot
    ...
    async def _watch_pairs(self) -> None:
        """Records which robots hold the pairs their models ask for, on the
        same tick a stay checks its lease. A robot's pairs dissolve when its
        nodes stop, so this is how a robot that is gone stops renewing, and a
        robot paired as another model never renews at all."""
    ...
    def _admit(self, request: attach.GoalRequest) -> attach.GoalDecision:
        ...
        with self._admit_lock:
            try:
                adopted = self._robots.admit(request.data.robot, entry, caller)
            except ValueError as error:
                return attach.GoalDecision.reject(str(error))
            if adopted:
    ...
    async def _stay(self, context: attach.GoalContext) -> None:
        ...
        have not been its model's for the lease, or when the engine takes it
        out.
