...
class CaptureOutcome(enum.Enum):
    ...
    renderer and the model's cameras disagree and no retry fixes it, the second
...
# camera before the renderer and the model's cameras are declared
# incompatible; first frames legitimately take a few updates to appear.
_MAX_CAPTURE_FAILURES = 300


class IsaacCameraSensor:
    """Renders the cameras of one robot's model through Replicator render
    ...
    def __init__(self, robot: str, root_prim: str, known, io) -> None:
        # The robot this rig is mounted on: every frame it publishes goes to
        # that robot's own camera pair.
        self._robot = robot
        self._root = root_prim
        # The model the robot joined as: the rig is the cameras of its entry,
        # each hanging from the prim its parent link is in this model's stage.
        self._known = known
    ...
    def setup(self) -> bool:
        ...
        parent link missing from a loaded robot raises, because that is an
        error of the model's entry no retry can fix."""
        if self._ready:
            return True
        try:
    ...
    def _define_camera_prim(self, stage, root, camera: CameraConfig) -> str:
        ...
        # The prim a camera hangs from: the one its parent link is in this
        # model's stage, wherever under the robot's prim it sits.
        parent_prim = self._known.prim_of(camera.parent_link)
        parent = next(
            (prim for prim in Usd.PrimRange(root) if prim.GetName() == parent_prim),
            None,
        )
        if parent is None:
            ...
            raise RuntimeError(
                ...
                f" under {self._root} as '{parent_prim}'; prims: {names}"
            )
        ...
        if outcome is CaptureOutcome.NO_FRAME:
            ...
            if self._capture_failures[name] >= _MAX_CAPTURE_FAILURES:
                raise RuntimeError(
                    ...
                    "consecutive reads; renderer output and the model's camera disagree"
                )
            return False
