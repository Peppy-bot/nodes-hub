"""Typed peppygen pairing IO for the Isaac Sim simulation.
...
in-order) and keeps the latest governed setpoint per limb of each robot in
thread-safe slots; the physics thread reads those and publishes each robot's
...
class SimTopicIO:
    ...
    def camera_robots(self) -> set[str]:
        ...
        for none. Read on the physics thread, once per state tick."""
        return {
            robot
            for robot, held in self._pairs.held().items()
            if held.rgb_cameras or held.rgbd_cameras
        }
