"""g1_node_mujoco entry point: MuJoCo G1 behind the shared G1 contract surface.

A physics thread owns the engine (mj_step + snapshot); the node's asyncio loop
consumes base velocity, serves the set_posture action, and publishes the state /
IMU / joint / odometry telemetry from the latest snapshot. This mirrors the
openarm sim engine's thread split.
"""
...
from .engine import MOTOR_COUNT, G1MujocoEngine, Snapshot
from .postures import UnknownPosture, sim_posture


class _LatestSlot:
    """Thread-safe latest-wins snapshot, written by the physics thread."""
...
def _physics_loop(
    ...
    if not headless:
        ...
        except Exception as exc:  # no display, missing GL: fall back to headless
            print(f"[g1-mujoco] viewer unavailable, running headless: {exc!r}")
    try:
...
async def _run_velocity_subscriber(node_runner: NodeRunner, engine: G1MujocoEngine, token):
...
async def _run_posture_action(node_runner: NodeRunner, engine: G1MujocoEngine, token):
    ...
    async def drive(ctx):
        try:
            ...
            print(f"[g1-mujoco] set_posture failed: {error!r}")
        finally:
            busy[0] = False
            ...
            try:
                ...
                await imu_pub.publish(
                    g1_imu.build_message(
                        s.quaternion, s.gyroscope, s.accelerometer, s.rpy, 25
                    )
                )
                await joints_pub.publish(
                    g1_joint_states.build_message(s.q, s.dq, s.tau_est, zeros_i, volts, modes)
                )
                ...
                print(f"[g1-mujoco] telemetry publish error: {exc!r}")
                break
...
async def setup(params: Parameters, node_runner: NodeRunner) -> list[asyncio.Task]:
    engine = G1MujocoEngine(kp=params.pd_kp, kd=params.pd_kd)
    slot = _LatestSlot(engine.snapshot())
    stop = threading.Event()
    physics = threading.Thread(
        target=_physics_loop,
        args=(engine, slot, float(params.physics_rate_hz), stop, params.headless),
        name="g1-mujoco-physics",
        daemon=True,
    )
    physics.start()
    ...
    async def on_shutdown():
        stop.set()
        print("[g1-mujoco] physics stopped")
