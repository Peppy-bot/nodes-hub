...
#
# Usage:
#   RCLONE_S3_ACCESS_KEY_ID=<key> RCLONE_S3_SECRET_ACCESS_KEY=<secret> bash sim_base_images/build_base_images.sh
set -euo pipefail
...
MUJOCO_IMAGE_REV="2"     # bump when MuJoCo image content changes without an upstream version bump
...
NODES_DIR="$(cd "${IMAGES_DIR}/.." && pwd)"

ISAAC_IMAGE="${IMAGE_NAMESPACE}/sim-isaac:${ISAAC_VERSION}-${ISAAC_IMAGE_REV}"
MUJOCO_IMAGE="${IMAGE_NAMESPACE}/sim-mujoco:${MUJOCO_VERSION}-${MUJOCO_IMAGE_REV}"
...
if (( BUILD_MUJOCO )); then
    ...
    docker buildx build \
        ...
        --secret id=rclone_secret,env=RCLONE_S3_SECRET_ACCESS_KEY \
        -t "${MUJOCO_IMAGE}" \
        -f "${IMAGES_DIR}/Dockerfile.mujoco" \
        --push \
        "${IMAGES_DIR}"
else
