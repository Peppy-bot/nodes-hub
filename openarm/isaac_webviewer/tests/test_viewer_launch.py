...
def test_setup_registers_shutdown_after_start_and_cleans_up_off_event_loop(startup, caplog):
    ...
    asyncio.run(run())
    startup.server_factory.assert_called_once_with(("0.0.0.0", 8210))
    ...
    assert any("listening on 0.0.0.0:8210" in message for message in messages)
...
def test_container_packages_the_server_beside_launch():
    ...
    assert 'exec python3 /opt/openarm_isaac_webviewer/launch.py "$@"' in runscript
