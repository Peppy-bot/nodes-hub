...
OPENARM_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
...
if (( BUILD_ISAAC )); then
    ...
    docker buildx build \
        ...
        -t "${ISAAC_IMAGE}" \
        -f "${OPENARM_DIR}/scripts/Dockerfile.isaac" \
        --push \
        "${OPENARM_DIR}"
else
...
if (( BUILD_MUJOCO )); then
    ...
    docker buildx build \
        ...
        --secret id=rclone_secret,env=RCLONE_S3_SECRET_ACCESS_KEY \
        -t "${MUJOCO_IMAGE}" \
        -f "${OPENARM_DIR}/scripts/Dockerfile.mujoco" \
        --push \
        "${OPENARM_DIR}"
else
...
sync_def_from "${OPENARM_DIR}/sim_isaac/apptainer.def" "${ISAAC_IMAGE}"
sync_def_from "${OPENARM_DIR}/sim_mujoco/apptainer.def" "${MUJOCO_IMAGE}"
...
echo "      (cd \"${OPENARM_DIR}/sim_mujoco\" && peppy node add . -sb)"
echo "      (cd \"${OPENARM_DIR}/sim_isaac\" && peppy node add . -sb)"
