...
physics reads it, so the seat server hands the work over here and waits for
the answer.
"""
...
class Edits:
    ...
    def drain(self) -> int:
        """Runs everything waiting, on the calling thread. Returns how many
        changes were made, so a caller that has to rebuild knows to."""
        made = 0
        while True:
            try:
