...
def test_the_cameras_outlive_the_views_they_stand_beside(bridge, monkeypatch):
    ...
    monkeypatch.setattr(module, "RobotLimbs", lambda robot, layout: _limbs_of(None, robot.instance))
...
def test_resolving_raises_for_a_robot_whose_joints_the_engine_cannot_drive(bridge):
    ...
    alpha.setup.side_effect = RuntimeError("sim_bridge.json5 references joints not on the articulation")
