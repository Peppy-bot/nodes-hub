...
def standing(name: str = "alpha") -> tuple[Registry, Caller]:
    ...
    robots = Registry()
    held = caller()
    robots.admit(name, "openarm_v2", held, 0.0)
    robots.stand(name, LIMBS, now_s=0.0)
    return robots, held


class TestJoining:
    def test_a_robot_reserves_its_name_and_stands_with_its_limbs(self):
        robots, held = standing()
        robot = robots.of_name("alpha")
        assert (robot.model, robot.caller) == ("openarm_v2", held)
        assert robot.limbs == LIMBS
    ...
    def test_a_name_another_robot_stands_under_is_refused(self):
        ...
        with pytest.raises(ValueError, match="already stands as 'alpha'"):
            robots.admit("alpha", "openarm_v2", caller("bravo_init_inst"), 0.0)
    ...
    def test_a_copy_re_registering_its_own_robot_takes_it_over(self):
        ...
        robots, held = standing()
        assert robots.admit("alpha", "openarm_v2", held, 5.0) is True
    ...
    def test_the_name_of_the_only_robot_is_what_an_uncopied_pair_belongs_to(self):
        ...
        robots.admit("bravo", "openarm_v2", caller("bravo_init_inst"), 0.0)
    ...
    def test_a_released_name_is_free_again(self):
        ...
        robots.admit("alpha", "openarm_v1", caller("bravo_init_inst"), 0.0)
...
class TestLeases:
    ...
    the robot still holds a limb pair."""

    def test_a_robot_holding_a_pair_marks_its_lease(self):
        robots, _ = standing()
