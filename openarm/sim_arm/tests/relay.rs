...
//! joint_link peers (backbone leader, simulation follower) played by generated
...
async fn relays_setpoints_and_states_between_backbone_and_simulation() -> peppygen::Result<()> {
    let (harness, mut mocks) = Harness::start(openarm_sim_arm::setup).await?;
...
async fn ready_latches_on_first_state_and_health_flows_only_while_fresh() -> peppygen::Result<()> {
    let (mut harness, mut mocks) = Harness::start(openarm_sim_arm::setup).await?;
...
async fn serves_ready_and_ingests_simulation_states_without_backbone_peer() -> peppygen::Result<()>
{
    ...
    let (mut harness, mocks) = Harness::start_with(config, openarm_sim_arm::setup).await?;
    assert!(
