...
USER_AGENT = "openarm_sim_mujoco/head_camera.py"
...
# The config rounds the rig to a tenth of a millimetre and seven decimals.
...
def check_chest_camera(rig: dict, cameras_config: Path) -> None:
    """The chest camera the sensor renders (config/cameras.json5) must be
    the pack's left eye: the same parent link, its position to a tenth of a
    millimetre, its orientation the same rotation. The config is kept by
    hand from the rig the derivation prints, and this refuses a pack bump
    or a config edit that leaves the stream looking out of the drawn lens."""
    from camera_common import load_camera_configs

    chest = next((c for c in load_camera_configs(cameras_config) if c.name == CHEST_CAMERA), None)
    if chest is None:
        raise RuntimeError(f"{cameras_config} declares no {CHEST_CAMERA!r} camera")
