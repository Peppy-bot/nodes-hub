...
from peppygen.consumed_actions.ready_posture import (
    move_to_ready as ready_posture_move_to_ready,
)
...
from xr_commander import config, publish, tls, video
...
async def setup(params: Parameters, node_runner: NodeRunner) -> list[asyncio.Task]:
    ...
    video.assert_unique_track_ids(tracks)
    ...
    session = XrSession(
        ...
        camera_views=video.camera_views(tracks),
    )
    ...
    tasks = [
        asyncio.create_task(
            publish.run_ready_button(
                node_runner,
                action_module=ready_posture_move_to_ready,
                session=session,
                settings=settings,
                token=token,
            )
        ...
        tasks.append(
            asyncio.create_task(
                publish.drain_pose_states(
                    node_runner, wiring.pose_states, measured, handedness, token
                )
            )
        )
        tasks.append(
