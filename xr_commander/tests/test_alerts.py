...
def test_no_alerts_means_no_banner():
    assert ActiveAlerts(banner_from=WARNING).banner() is None


def test_an_alert_names_its_source_severity_and_message():
    active = ActiveAlerts(banner_from=WARNING)
    raise_alert(active)
    text, severity = active.banner()
    assert text == "LEFT ARM J2 CRITICAL: holding 96% of rated torque"
    assert severity == 2


def test_an_undefined_severity_is_refused():
    # The contract tops out at 3; accepting more would let a malformed
    # producer outrank a genuine fault banner.
    with pytest.raises(ValueError):
        ActiveAlerts(banner_from=WARNING).update(
            "left arm j2", "motor_overload", 4, "boom"
        )


def test_an_alert_without_an_identity_is_refused():
    # The commander rejects these too; a banner reading " CRITICAL: ..."
    # names nothing the operator can act on.
    with pytest.raises(ValueError):
        ActiveAlerts(banner_from=WARNING).update("", "motor_overload", 2, "boom")
    with pytest.raises(ValueError):
        ActiveAlerts(banner_from=WARNING).update("left arm j2", "", 2, "boom")


def test_a_severity_zero_clear_removes_the_banner():
    active = ActiveAlerts(banner_from=WARNING)
    raise_alert(active)
    assert active.banner() is not None
    active.update("left arm j2", "motor_overload", 0, "recovered")
    assert active.banner() is None
...
def test_a_clear_only_removes_its_own_identity():
    ...
    active.update("left arm j2", "motor_overload", 0, "recovered")
    text, severity = active.banner()
    assert text.startswith("RIGHT ARM J1")
    assert severity == 1


def test_the_worst_alert_wins_the_banner():
    active = ActiveAlerts(banner_from=WARNING)
    ...
    raise_alert(
        ...
        source="right arm j7",
        severity=3,
        message="overload: the motor cut out and the joint is limp",
    )
    text, severity = active.banner()
    assert text.startswith("RIGHT ARM J7 FAULT")
    assert severity == 3


def test_a_stale_alert_stops_being_drawn(monkeypatch):
    # A producer that dies mid-alert must not leave its banner burned into
    # the video forever; the fixed window is 3x the contract's 2 s re-emit
    # ceiling.
    clock = {"now": 100.0}
...
def test_the_banner_only_covers_the_top_of_the_frame():
    frame = np.zeros((200, 320, 3), dtype=np.uint8)
    drawn = draw_banner(frame, "LEFT ARM J2 WARNING: holding 93%", 1)
    assert drawn.shape == (200, 320, 3)
    # Banner across the top, untouched video below it.
    assert drawn[:5].any()
    assert not drawn[60:].any()


def test_a_narrow_frame_still_fits_the_whole_message():
    # The text scales down rather than running off the edge, so the operator
    # can read the alert on any camera geometry.
    narrow = draw_banner(
        np.zeros((120, 160, 3), dtype=np.uint8), "LEFT ARM J2 FAULT: limp", 3
    )
    banner_height = int(120 * 0.14)
    text_columns = np.nonzero((narrow[:banner_height] == 255).all(axis=2).any(axis=0))[
        0
    ]
    assert text_columns.size > 0
    assert text_columns.min() > 0
    assert text_columns.max() < 159


def test_severity_picks_the_banner_color():
    warning = draw_banner(np.zeros((100, 200, 3), dtype=np.uint8), "X", 1)
    fault = draw_banner(np.zeros((100, 200, 3), dtype=np.uint8), "X", 3)
    assert tuple(warning[0, 0]) != tuple(fault[0, 0])


def test_a_warning_stays_off_the_video_when_a_panel_lists_it():
    # The banner cannot be looked away from, so with a panel carrying every
    # alert the video is only interrupted by what must stop the operator.
    active = ActiveAlerts(banner_from=CRITICAL)
    raise_alert(active, severity=1, message="warm")
    assert active.banner() is None
    assert len(active.active()) == 1


def test_a_warning_reaches_the_video_when_nothing_else_can_show_it():
    active = ActiveAlerts(banner_from=WARNING)
    raise_alert(active, severity=1, message="warm")
    assert active.banner() is not None


def test_a_fault_reaches_the_video_whatever_the_threshold():
    active = ActiveAlerts(banner_from=CRITICAL)
    raise_alert(active, severity=3, message="limp")
    assert active.banner()[1] == 3


def test_an_undefined_banner_threshold_is_refused():
    # Zero would banner a cleared alert; above the ceiling would banner none.
    with pytest.raises(ValueError):
        ActiveAlerts(banner_from=0)
    with pytest.raises(ValueError):
        ActiveAlerts(banner_from=4)
...
def test_a_stale_alert_leaves_the_list_too(monkeypatch):
    # The panel purges on the same window as the banner, so a dead producer
    # cannot leave a line the operator reads as current.
    clock = {"now": 100.0}
    monkeypatch.setattr("xr_commander.alerts.time.monotonic", lambda: clock["now"])
    active = ActiveAlerts(banner_from=CRITICAL)
    raise_alert(active)
...
def test_a_long_message_is_truncated_rather_than_shrunk_to_a_smear():
    ...
    # 14% of the operator's video carrying no information.
...
def test_the_banner_says_how_much_else_is_wrong():
    # On a stack with no status panel the banner is the only channel, so one
    # dead joint and a whole dead arm must not read the same.
    active = ActiveAlerts(banner_from=WARNING)
    for joint in range(1, 8):
        active.update(f"left arm j{joint}", "motor_condition", 3, "limp")
    text, severity = active.banner()
    assert severity == 3
    assert "+6 more" in text


def test_a_single_alert_banner_carries_no_count():
    active = ActiveAlerts(banner_from=WARNING)
    raise_alert(active)
    text, _ = active.banner()
...
def test_an_unwired_alert_slot_is_distinguishable_from_a_quiet_robot():
    unwired = ActiveAlerts(banner_from=WARNING, producers_bound=False)
    wired = ActiveAlerts(banner_from=WARNING, producers_bound=True)
