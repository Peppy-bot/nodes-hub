...
def test_ready_duration_allows_zero_for_fastest_and_refuses_the_rest():
    assert config.from_parameters(
        params(ready_move_duration_s=0.0)
    ).ready_move_duration_s == pytest.approx(0.0)
    for bad in (-1.0, math.nan, math.inf):
        with pytest.raises(ValueError, match="ready_move_duration_s"):
            config.from_parameters(params(ready_move_duration_s=bad))
