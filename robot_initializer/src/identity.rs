...
pub struct Identity {
    ...
    pub robot: String,
    /// The model this robot is, as a simulation's catalogue names it and as
...
impl Identity {
    /// The identity of the robot `runner` runs, a robot of `model`.
    pub fn of(runner: &NodeRunner, model: &str) -> Self {
    ...
    fn named(copy: Option<&str>, instance_id: &str, core_node: &str, model: &str) -> Self {
        Self {
...
mod tests {
    ...
    fn a_robot_in_a_copy_runs_under_the_copy_name() {
        assert_eq!(
            Identity::named(Some("bravo"), "bravo_init_inst", "jetson-1", "openarm_v2"),
            Identity {
    ...
    fn a_robot_outside_a_copy_runs_under_its_instance_id() {
        assert_eq!(
            Identity::named(None, "init_inst", "jetson-1", "so101"),
            Identity {
                robot: "init_inst".to_owned(),
                model: "so101".to_owned(),
                core_node: "jetson-1".to_owned(),
            }
        );
    }
