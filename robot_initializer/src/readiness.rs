...
//! answers for it reports ready. On a real robot that is whatever drivers
//! answer for its limbs, bound on `limbs`: one per limb on an OpenArm, the
//! single `so101_follower` on an SO-101. On a simulated one it is the
//! simulation standing it, bound on `simulation`, which answers for every
//! limb at once.
