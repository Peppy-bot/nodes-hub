...
def known_model(cameras, world_links=(_WELDED_LINK,), link_bodies=None, camera_lights=True):
    """A model whose entry carries these cameras, with what this engine knows
    about where they hang and how the scene is lit."""
    entry = ModelEntry(
        model="test", arms=(), grippers=(), cameras=tuple(cameras), start_posture={}
    )
    return MujocoModel(
        entry=entry,
        scene="scene.xml",
        world_links=frozenset(world_links),
        link_bodies=link_bodies or {},
        arm_gains=None,
...
def compile_model_with_cameras(path, cameras, **engine):
    """The scene at path compiled with the cameras on, as the launcher loads it."""
    spec = mujoco.MjSpec.from_file(str(path))
    add_cameras(spec, known_model(cameras, **engine), path)
    return spec.compile()
...
def test_compile_attaches_a_camera_to_the_body_its_parent_link_is_mapped_to(scene):
    """The entry names the URDF link, and the model's MJCF may call that
    link's body something else."""
    model = compile_model_with_cameras(
        scene, [color_camera(parent="base_link")], link_bodies={"base_link": "link_a"}
    )
    body = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "link_a")
    assert camera_body(model, "wrist") == body


def test_compile_hangs_a_world_link_camera_from_the_world_body(scene):
    model = compile_model_with_cameras(scene, [rgbd_camera()], world_links=[_WELDED_LINK])
    assert mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, _WELDED_LINK) == -1
    assert camera_body(model, "chest") == 0


def test_compile_keeps_a_world_link_camera_on_the_body_a_scene_gives_that_link(scene):
    model = compile_model_with_cameras(
        scene, [color_camera(parent="link_a")], world_links=["link_a"]
    )
    body = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "link_a")
    assert camera_body(model, "wrist") == body


@pytest.mark.parametrize("parent", ["no_such_link", _WELDED_LINK])
def test_compile_rejects_a_parent_link_that_is_neither_a_body_nor_a_world_link(scene, parent):
    with pytest.raises(RuntimeError) as excinfo:
        compile_model_with_cameras(scene, [color_camera(parent=parent)], world_links=[])
...
def test_a_scene_no_model_asks_to_light_keeps_its_own_lighting(scene):
    ...
    assert model.vis.headlight.diffuse.tolist() == baked.vis.headlight.diffuse.tolist()
...
def test_a_wedged_renderer_surfaces_on_the_physics_thread(scene, monkeypatch, clock):
    ...
    try:
        sensor.snapshot(np.zeros(model.nq))
        sensor.start()
        # The thread takes its beat at the top of the iteration that then
        # blocks inside render(), so the renderer reaching WedgedRenderer is
        # the point after which no further beat can land, whatever the host
        # does next. Aging the clock past the bound from there is the wedge.
        assert wedged.wait(timeout=5.0)
        clock.set(_HEARTBEAT_TIMEOUT_S + 1.0)
