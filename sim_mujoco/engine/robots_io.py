...
class RobotsIO:
    ...
    async def _watch_pairs(self) -> None:
        """Records whether the robot holds the pairs its model asks for, on
        the same tick its stay checks its lease. Its pairs dissolve when its
    ...
    def _admit(self, request: attach.GoalRequest) -> attach.GoalDecision:
        ...
        must be one this engine stands; the spot is the scene's own; and the
        only robot standing may be this caller's own. Admitting reserves the
        ...
        try:
            adopted = self._robots.admit(request.data.robot, entry, caller, self._loop.time())
        except ValueError as error:
            return attach.GoalDecision.reject(str(error))
        if adopted:
    ...
    async def _stay(self, context: attach.GoalContext) -> None:
        ...
        when its pairs have not been its model's for the lease, or when the
        engine stops.
        ...
        if robot.standing():
            ...
            try:
                await asyncio.wrap_future(self._stands.stand(self._models.of(model), name))
    ...
    def _mismatch(self, robot: Robot):
